...
newsAnalyst = Agent(
  ...
  You understand news, theirs titles and information, but you look at those with a health dose of skepticism. You consider also the source of the news articles.
""",
  verbose=True,
  llm= llm,
  max_iter= 10,
...
stockAnalystWrite = Agent (
  ...
  goal="""Analyse the trends price and write an insighfull compelling and informative 3 paragraph long newsletter based on the stock report and price trend.""",
...
#inputs = {
  #  'ticket': 'AAPL',
   # 'current_date': datetime.now().strftime('%Y-%m-%d')  # Include current_date in inputs
#}
